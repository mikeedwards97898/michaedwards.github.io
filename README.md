# Michael Edwards - Business Analyst Portfolio

Welcome to my professional portfolio showcasing data analytics projects, SQL expertise, and business intelligence solutions.

##  Portfolio Website
**Live Site:** [https://mikeedwards97898.github.io/michaedwards.github.io/](https://mikeedwards97898.github.io/michaedwards.github.io/)

##  Featured Projects

### 1. Customer Analytics SQL Project
**File:** `SQL_Customer_Analytics_Project.md`

Comprehensive SQL analysis demonstrating proficiency in subscription business metrics:
- **Customer Acquisition Cost (CAC)** analysis
- **Lifetime Value (LTV)** calculations
- **Churn rate** analysis and prediction
- **Cohort performance** tracking
- **Revenue forecasting** models
- **Customer segmentation** strategies

**Skills Demonstrated:**
- Advanced SQL (joins, CTEs, window functions)
- Business metrics calculation
- Data-driven decision making
- Statistical analysis
- Database design

### 2. Budget Process Optimization
**Organization:** Houston Independent School District  
**Impact:**
- ✅ 30% reduction in processing time
- ✅ 100% regulatory compliance maintained
- ✅ $50M budget managed across 5 campuses
- ✅ Real-time KPI tracking system implemented

**Technologies:** Excel (Advanced), Data Analytics, Process Mapping

### 3. Transportation Management System Integration
**Organization:** Brock Group  
**Impact:**
- ✅ 40% reduction in manual data entry
- ✅ 25% decrease in operational costs
- ✅ 75% faster report generation
- ✅ Enterprise-wide TMS implementation

**Technologies:** TMS Software, Excel Automation, Project Management

### 4. NEMT Operational Analytics Dashboard
**Organization:** Modivcare  
**Impact:**
- ✅ Real-time operational monitoring for executives
- ✅ Cost-saving opportunities identified through data analysis
- ✅ Automated quarterly market assessments
- ✅ Improved data-driven decision making

**Technologies:** Tableau, SQL, AWS Cloud, Data Modeling

##  Excel Templates


**Use Cases:**
- Customer acquisition tracking
- Revenue forecasting
- Performance monitoring
- Executive reporting
- Business analysis

##  Technical Skills

**Analytics & Visualization:**
- Tableau
- Excel (Advanced)
- SQL
- Data Modeling
- KPI Development

**Cloud & Infrastructure:**
- AWS Cloud Services
- Cloud-Based Reporting
- Data Storage Solutions
- Process Automation

**Project Management:**
- Agile/Scrum Methodologies
- Jira
- Process Improvement
- Cross-Functional Leadership

**Security & Compliance:**
- Vulnerability Management
- Regulatory Compliance
- Risk Assessment

##  Contact

**Email:** maedwards880@gmail.com  
**Phone:** 346-306-9703  
**Location:** Houston, Texas  
**Portfolio:** [https://mikeedwards97898.github.io/michaedwards.github.io/](https://mikeedwards97898.github.io/michaedwards.github.io/)

##  Professional Experience

**Business Analyst II** | Modivcare | Nov 2024 - Present
- SQL-based operational data analysis for NEMT sector
- Tableau dashboard development for executive reporting
- AWS Cloud integration for automated workflows

**Finance Coordinator II** | Houston ISD | Mar 2023 - Oct 2024
- $50M budget management and optimization
- KPI tracking system development
- 30% improvement in processing efficiency

**Senior Business Analyst** | Brock Group | Mar 2021 - Feb 2023
- TMS implementation and integration
- Cost analysis automation (75% time reduction)
- 40% reduction in manual processes

##  Education

**Bachelor of Arts in Business Administration**  
Sam Houston State University | Graduated May 2014

##  License

All projects and templates are created by Michael Edwards. Feel free to use the templates for personal or professional use.


